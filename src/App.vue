...
<template>
  <div class="flex md:flex-col flex-col gap-16 mt-64 md:mt-0 md:gap-16 items-center">
    <iconLogo />
    <div class="flex flex-col md:flex-row md:gap-14 md:w-[500px] gap-14 p-7 md:p-9 rounded-3xl bg-white">
      <!-- Content here -->
      ...
      <div class="flex flex-col gap-8">
        <div class="flex flex-col gap-2">
          ...
          <div class="flex items-center p-2 rounded space-x-[40px] px-4 gap-3 input-feild">
            <IconDollar />
            <input id="bills" v-model="billAmount" class="outline-none bg-transparent text-right" type="number" />
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2">
          <div class="flex items-center justify-between">
            ...
            <span v-show="number_people === 0" :class="'text-xs text-red-600' + (number_people === 0 ? '' : '')">Can't be zero</span>
          </div>
          <div :class="'flex items-center rounded space-x-[40px] p-2 px-4 gap-3 input-feild' + (number_people === 0 ? ' border-red-600 border-[0.5px]' : '')">
            <iconPerson />
            <input id="num_people" class="outline-none bg-transparent text-right" v-model="number_people" type="number" />
          </div>
        </div>
      </div>
      <!-- End Input -->
      ...
      <div class="flex flex-col md:gap-24 gap-10 rounded-2xl right-52 bg-cyan-900 p-5 md:p-8">
        <div class="flex flex-col gap-5">
          <div class="flex items-center gap-24">
            ...
            <h1 class="md:text-4xl text-3xl">${{ tipAmountPerPerson ? tipAmountPerPerson.toFixed(2) : '0.00' }}</h1>
          </div>
          <div class="flex items-center gap-28">
            ...
            <h1 class="md:text-4xl text-3xl">${{ TotalPerPerson ? TotalPerPerson.toFixed(2) : '0.00' }}</h1>
          </div>
        </div>
